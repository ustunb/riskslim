...
from riskslim.bounds import chained_updates
from riskslim.opt.mip import convert_to_risk_slim_solution
...
class PolishAndRoundCallback(HeuristicCallback):
    ...
    def __call__(self):
        ...
        if heuristic_update:
            ...
            proposed_values, proposed_objval = convert_to_risk_slim_solution(indices = self.indices, rho = best_solution, objval = best_objval)
            proposed_solution = SparsePair(ind = np.arange(self.indices['n_variables']), val = proposed_values)
